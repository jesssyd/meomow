...
export default function CatDetailScreen() {
  ...
  useEffect(() => {
    loadCat();
      ...
      console.log('Attempting to save cat:', cat.name); // Debug log

  const loadCat = async () => {
    if (!id) return;
    
    setLoading(true);
        console.log('Updating existing cat at index:', catIndex); // Debug log
    const catData = await CatStorage.getCatById(id);
    setCat(catData);
    setLoading(false);
        console.log('Adding new cat'); // Debug log
  };

  const handleEdit = () => {
      console.log('Saving to AsyncStorage...'); // Debug log
    if (cat) {
